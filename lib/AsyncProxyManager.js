...
class AsyncProxyManager {
    ...
    _startHttpServer() {
        ...
        {
            KalturaLogger.log('httpPort not defined, http server will not initialize');
            return;
        }
    ...
    _startHttpsServer() {
        ...
        {
            KalturaLogger.log('httpsPort not defined, http server will not initialize');
            return;
        }
    ...
    _handleCall(request, response) {
        ...
        KalturaRequestUtils.buildRequestParams(request)
            ...
            .then(function(requestHandler) {
                _this._cacheManager.cacheCall(requestHandler.cacheKey, request, requestHandler);
            })
            .then(function() {
    ...
    _getRequestHandler(request) {
        ...
        return new Promise(function (resolve, reject) {
            _this._handlers.forEach(function(h) {
                if( isSubset(request.requestParams, h.identifier) )
                    return resolve(h);
            });
    ...
    _generateCacheKey(requestHandler, request) {
        return new Promise(function (resolve, reject) {
            ...
            requestHandler.cacheKeyParams.forEach(function(keyParam) {
                if(!request.requestParams[keyParam]) {
