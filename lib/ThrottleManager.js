...
class ThrottleManager {
    ...
    constructor() {
        try {
            ...
            this.httpsHeader = null;
            if(config.has('throttle.httpsHeader'))
                this.httpsHeader = config.get('throttle.httpsHeader');
            ...
            this.callApi = rateLimiter(function (request, sendExtraParams) {
                try {
                    ...
                    let cdnApiHost = config.get('cdn_api_host.cdn_api_host_http');
                    if(request.protocol.toLowerCase() == 'https' || (_this.httpsHeader && request.headers[_this.httpsHeader] == "ON"))
                        cdnApiHost = config.get('cdn_api_host.cdn_api_host_https');
