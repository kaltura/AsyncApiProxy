...
const uidGenerator = require('node-uuid');
...
KalturaLogger = {
	...
	access: function(request, response){
		...
		response.log = function(msg){
			KalturaLogger.log('Request [' + response.sessionId + '] ' + msg, getStack());
		};
		response.dir = function(object){
			KalturaLogger.dir(object, getStack(), 'Request [' + response.sessionId + '] ');
		};
		response.error = function(msg){
			KalturaLogger.error('Request [' + response.sessionId + '] ' + msg, getStack());
		};
		response.debug = function(msg){
			KalturaLogger.debug('Request [' + response.sessionId + '] ' + msg, getStack());
		};
