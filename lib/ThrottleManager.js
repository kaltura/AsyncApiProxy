            ...
            this.callApi = rateLimiter(function (request) {
                try {
                    ...
                    function ThrottleResponseCB(error, response, body) {
                        ...
                        if (error) {
                            request.error(`Failed API CALL: [X-ME: ${request.requestParams['X-Me']} , SessionId: ${request.requestParams.sessionId} \n\r ${util.inspect(error)}`);
                            if (request.callback)
                        ...
                        if (response.statusCode === 200) {
                            request.log(`Successful API CALL: X-ME:[${request.requestParams['X-Me']}] , SessionId:[${request.requestParams.sessionId}], X-ME:[${response.headers['x-me']}] , X-Kaltura-Session:[${response.headers['x-kaltura-session']}] `);
                            request.log(`[${request.requestParams.sessionId}] Got response from API server - body [${body}]`);
                            if (request.callback)
                                request.callback(response);
                        }
                        else {
                            request.error(`Failed API CALL: X-ME:[${request.requestParams['X-Me']}] , SessionId:[${request.requestParams.sessionId}], X-ME:[${response.headers['x-me']}] , X-Kaltura-Session:[${response.headers['x-kaltura-session']}]\n\r ${util.inspect(error)}`);
                            if (request.callback)
                                request.callback(response,error);
                        }
                    }
                    ...
                    request.error(`[${request.requestParams.sessionId}] Error sending api call - " + ${util.inspect(err)}`);
                    if (request.callback)
