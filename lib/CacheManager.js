...
class CacheManager {
    ...
    cacheCall(cacheKey, request, requestHandler) {
        ...
        return new Promise(function (resolve, reject) {
            if (!requestHandler || !requestHandler.ttl) {
                KalturaLogger.debug("Not Caching call: " + util.inspect(requestHandler));
                return reject("Will not Cache call - missing mandatory data for call handler\n");
            }

            if (!cacheKey) {
                KalturaLogger.debug("Missing Cache Key ");
                return reject("Will not Cache request - Missing Cache Key\n");
            }

            if (!request) {
                KalturaLogger.debug("Missing request");
            ...
            try {
                ...
                if (requestHandler.extendTtl) {
                    KalturaLogger.debug("[" + new Date().getTime() + "] Handling cacheKey: [" + cacheKey + "] request:[ " + util.inspect(request) + "]");
                ...
                // handling aggregation type request - extending ttl if exists in cache.
                    ...
                    KalturaLogger.debug("[" + new Date().getTime() + "] Handling cacheKey: [" + roundedCacheKey + "] request:[ " + util.inspect(request) + "]");
    ...
    _CachedCallCallback(key, request){
        KalturaLogger.debug('Handling expired item. cacheKey: ['+ key + '] request:[' + util.inspect(request)+ ']');
